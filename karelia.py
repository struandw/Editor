...
class bot:
    ...
    def __init__(self, name, room, renegade=False):
    ...
    def send(self, message, parent=''):
        ...
        """
        if not self.paused:
            if isinstance(message, dict) and not (message["type"] == "send" and self.renegade):
                self.conn.send(json.dumps(message))
            elif len(message) > 0 and not self.renegade:
